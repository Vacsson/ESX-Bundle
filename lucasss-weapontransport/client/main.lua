...
Citizen.CreateThread(function()
	while ESX == nil do
		TriggerEvent('esx:getSharedObject', function(obj)
			...
			Marker.AddMarker('cargo_start', Config.StartPosition, 'Press ~INPUT_CONTEXT~ ~w~to start a ~b~cargo ~y~($' .. Config.Price .. ')', nil, 0, function()
									...
									if state == true then
										ESX.TriggerServerCallback('revenge-weapontransport:buyCargo', function(success)
											if success == true then
												...
												TriggerEvent('esx:showNotification', '~r~You do not have enough money. ($' .. v.Price .. ')')
											end
										end, Config.Price)
									end
								end)
							else
								TriggerEvent('esx:showNotification', '~r~There must be atleast 3 police in duty.')
							end
						end)
					else
						TriggerEvent('esx:showNotification', '~r~There has recently been a cargo. Wait ~b~' .. cooldown .. ' ~r~seconds.')
					end
				end, 'cargo')
			end)
		end)
		...
		if CurrentTransport ~= nil then
			...
			if (CurrentTransport.timestamp + Config.BlipFollowing) > GetGameTimer() then
				...
				if CurrentTransport.updatingBlip == true then
					...
					NotifyPolice('We lost the cargo on the GPS, find it!')
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentTransport ~= nil then
			...
			drawTxt(0.90, 1.40, 1.0, 1.0, 0.4, ('Seconds left on the cargo: ~b~' .. secondsLeft .. ' (' .. math.floor((secondsLeft / 60)) .. 'm)'), 255, 255, 255, 255, false)    						
		end
	end
end)
...
function StartTransport()
	...
	Marker.AddMarker('cargo_deliver', deliverPosition, 'Press ~INPUT_CONTEXT~ ~w~to deliver the ~b~cargo', nil, 0, function()
		DeliverTransport()
	...
	local index = 0
	local randomizedVehicle = math.random(0, vehicles)
	local cargo = {}

	for k,v in pairs(Config.Cargos) do
		if index == randomizedVehicle then
			randomizedVehicle = k
			cargo = v
	...
	NotifyPolice('WARNING: A cargo with suspicious items in it has been spotted and will be delivered at a location within 60 minutes. ')
	...
	if vehicle ~= nil then
		if CurrentTransport ~= nil then
			if vehicle == CurrentTransport.vehicle then
				...
				TriggerEvent('esx:showNotification', 'Wrong vehicle!')
			end
		end
	end
end
...
function FailTransport()
	...
	NotifyPolice('The cargo delivery has been cancelled. Good job!')
end
